...
class TestViews(TestCase):
    ...
    def test_remove_from_wishlist(self):
        ...
        product = Product.objects.create(
            ...
            price=11.2, sku=46576)
        self.remove_from_wishlist_url = reverse('remove_from_wishlist',
                                                args=[product.product_id])
        session = self.client.session
        session['wishlist_bag'] = {product.product_id: True}
        session['wishlist_item_id'] = [product.product_id]
        session.save()
        response = self.client.get(self.remove_from_wishlist_url,
                                   HTTP_REFERER='happy-pet.com')
        self.assertEquals(response.status_code, 302)
