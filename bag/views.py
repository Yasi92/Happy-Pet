    ...
    if size and color:
        if item_id in list(bag.keys()):
            if item in bag[item_id]['items_by_filter'].keys():
                ...
                messages.success(
                    request, f'Updated {size} {color} { product.name } \
                    quantity to {bag[item_id]["items_by_filter"][item]}')

            else:
                bag[item_id]['items_by_filter'][item] = quantity
                messages.success(
                    request, f'Added size {size} color\
        ...
        if item_id in list(bag.keys()):
            if size in bag[item_id]['items_by_size'].keys():
                ...
                messages.success(
                    request, f'Updated size {size} { product.name } \
        ...
        if item_id in list(bag.keys()):
            if color in bag[item_id]['items_by_color'].keys():
                ...
                messages.success(
                    request, f'Updated color \
                    {color} { product.name } quantity to\
                    {bag[item_id]["items_by_color"][color]}')

            else:
                bag[item_id]['items_by_color'][color] = quantity
                messages.success(
                    request, f'Added color \
                    {color} { product.name } to your bag')
...
def adjust_bag(request, item_id):
    ...
    if size and color:
        ...
        if quantity > 0:
            bag[item_id]['items_by_color'][color] = quantity
            messages.success(
                request, f'Updated color {color} { product.name } \
    ...
    try:
        ...
        if size and color:
            ...
            if not bag[item_id]['items_by_filter']:
                bag.pop(item_id)
            messages.success(
                request, f'Removed size {size} color \
                {color} {product.name} from your bag')
            ...
            messages.success(
                ...
                {size} {product.name} from your bag')
